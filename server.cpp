...
void handle_request(ClientConnection& conn)
{
    HTTP http;
    ...
    http.return_res(conn.response_header, conn.filename);
    ...
    while(true)
    {
        ...
        for(int i = 0; i < nfds; i++)
        {
            ...
            else if(events[i].events & EPOLLIN)//read
            {
                ...
                ssize_t has_read = read(fd, buf, BUFSIZE);
                if(has_read <= 0)
                {
                    close(fd);
                    events[i].data.fd = -1;
                ...
                else
                {
                    ...
                    for(int j = 0; j < CONSIZE; j++)//store this fd's read
                    {
                        if(connections[j].fd == fd)
                        {
                            ...
                            handle_request(connections[j]);//prase the request and write the response to response
                            break;
                        }
                    }
                }
